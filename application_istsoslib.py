...
def executeSos(environ, start_response):
    ...
    try:
        ...
        if not sosConfig.user.allowedService(sosConfig.schema):
            raise sosException.SOSException(
                  "NoApplicableCode", "", "You are not authorized to access the '%s' instance" % sosConfig.schema)
                  
        elif req_filter.request in ['insertobservation','registersensor']:
            # If hybrid mode enable check authorizations 
            # > http://istsos.org/en/latest/doc/security.html#hybrid-w-o-insertobservation-and-registersensor
            if config.hybrid and (not 'HTTP_AUTHORIZATION' in environ):
                raise sosException.SOSException(
                      "NoApplicableCode", "", "In hybrid mode, you are not authorized to execute %s requests on this server" % req_filter.request)
                      
            # Only Admin, Network Managers and Data Manager con execute insertobservation or registersensor
            elif not sosConfig.user.isAdmin() and not sosConfig.user.isNetworkManager() and not sosConfig.user.isDataManager():
                raise sosException.SOSException(
                      "NoApplicableCode", "", "You are not authorized to execute %s requests on this server" % req_filter.request)
        ...
        if str(environ['REQUEST_METHOD']).upper()=='GET':
            ...
            if requestObject.has_key("attachment"):
                response_headers.append(
                    ("Content-Disposition", "attachment; filename=%s" % requestObject["attachment"])
                )
