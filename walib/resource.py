...
class waResource(object):
    ...
    def __init__(self, waEnviron, loadjson=True):
        ...
        if self.method in ["POST", "PUT"] and loadjson is True:
            import json
    ...
    def validateGet(self):
        ...
        raise Exception(
            (
                "%s.validateGet method is not implemented!"
    ...
    def validatePost(self):
        ...
        raise Exception(
            (
                "%s.validatePost method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def validatePut(self):
        ...
        raise Exception(
            (
                "%s.validatePut method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def validateDelete(self):
        ...
        raise Exception(
            (
                "%s.validateDelete method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def executeGet(self):
        ...
        raise Exception(
            (
                "%s.executeGet method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def executePost(self):
        ...
        raise Exception(
            (
                "%s.executePost method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def executePut(self):
        ...
        raise Exception(
            (
                "%s.executePut method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def executeDelete(self):
        ...
        raise Exception(
            (
                "%s.executeDelete method is not implemented!"
            ) % self.__class__.__name__)
    ...
    def setData(self, data):
        ...
        self.response['total'] = len(data) if isinstance(data, list) else 0
...
class waResourceAdmin(waResource):
    """
    Extension class of istSOS Web Admin REST operations that
    manages Wab Admin Configuration
    """
    def __init__(self, waEnviron, loadjson=True):
...
class waResourceService(waResourceAdmin):
    def __init__(self, waEnviron, service=None, loadjson=True):
        ...
        defaultCFGpath = os.path.join(
            self.waconf.paths["services"], "default.cfg")
        if not os.path.isfile(defaultCFGpath):
            raise Exception(
                (
                    "istsos [default] configuration "
                    "file not found in %s."
                ) % (defaultCFGpath)
            )

        if not (self.service is None or self.service == 'default'):
            serviceCFGpath = os.path.join(
                self.waconf.paths["services"],
                "%s" % self.service,
                "%s.cfg" % self.service
            )
            ...
            self.servicepath = os.path.join(
                self.waconf.paths["services"], "%s" % self.service)

            sensormlpath = os.path.join(
                self.waconf.paths["services"], "%s" % self.service, "sml")
            ...
            virtualpath = os.path.join(
                self.waconf.paths["services"], "%s" % self.service, "virtual")
            if not os.path.isdir(virtualpath):
                raise Exception(
                    (
                        "istsos [%s] virtual procedures "
                        "folder not found in %s."
                    ) % (
                        self.service,
                        virtualpath
                    )
        ...
        if self.service is None or self.service == 'default':
...
class waResourceConfigurator(waResourceService):
    ...
    def executeGet(self):
        ...
        for key in self.template:
            ...
            if not self.template[key][1] in temp:
                raise Exception(
                    (
                        "Configuration error: value \"%s\" "
                        "not present in section \"%s\", check "
        ...
        for s in self.sections:
            data['default'] = (
                data['default'] and
                self.serviceconf.get(s)["default"]
            )
        self.setData(data)
