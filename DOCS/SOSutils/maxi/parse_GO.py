...
def getElemAtt(node,att,mode='p'):
    if att in node.attributes.keys():
        return str(node.getAttribute(att).encode( "utf-8" ))
    else:
        if mode == 'p':
            return None
        else:
        ...
        if len(res)==1:
            ...
            #case DataArray
                ...
                for id, field in enumerate(fields):
                    ...
                    if len(values)==1:
                        txtvals = getElemTxt(values[0]).strip("\n ")
            ...
            #case simple om:result
                ...
                self.result[sosConfig.urn["time"]]=[self.samplingTime]
                uom = getElemAtt(res[0],"uom")
                vals = [getElemTxt(res[0])]
                self.result[sosConfig.urn["phenomena"]+self.oprName+"@"+uom]=vals
