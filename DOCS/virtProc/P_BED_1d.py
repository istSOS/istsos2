    ...
    def __init__(self,filter,pgdb):
        ...
        if self.filter.eventTime:
            ...
            if len(self.filter.eventTime[0]) == 2:
                self.filter.eventTime[0][0] = iso.datetime_isoformat(iso.parse_datetime(self.filter.eventTime[0][0]).replace(hour=0, minute=0, second=0, microsecond=0))
            if len(self.filter.eventTime[0]) == 1:
                self.filter.eventTime[0][0] = iso.datetime_isoformat(iso.parse_datetime(self.filter.eventTime[0][0]).replace(hour=0, minute=0, second=0, microsecond=0)-self.d)
                self.filter.eventTime[0][1] = iso.datetime_isoformat(iso.parse_datetime(self.filter.eventTime[0][0]).replace(hour=0, minute=0, second=0, microsecond=0))
        ...
        if not self.filter.eventTime:
            ...
            ot = self.st[1].replace(hour=0, minute=0, second=0, microsecond=0)
            self.filter.eventTime[0][0] = iso.datetime_isoformat(ot-self.d)
            self.filter.eventTime[0][1] = iso.datetime_isoformat(ot)
            #reset the inputs
            (self.pioggia,self.st) = self.setSOSobservationVar("P_BED","rainfall",True)
        
        self.st[0] = self.st[0].replace(hour=0, minute=0, second=0, microsecond=0) + self.d
        self.st[1] = self.st[1].replace(hour=0, minute=0, second=0, microsecond=0)
