...
from istSOS import sosException, sosUtils
    ...
    def __init__(self,sosRequest,method,requestObject):
        ...
        if method == "GET":
            ...
            if requestObject.has_key("offering"):
                self.offering = get_name_from_urn(requestObject["offering"],"offering")
            else:
            ...
            if requestObject.has_key("featureOfInterest"):
                ...
                if foi.find("<ogc:")>=0 and foi.find("<gml:")>=0:
                    ...
                    self.featureOfInterest = get_name_from_urn(foi,"feature")
            ...
            if requestObject.has_key("aggregateFunction"):
                if self.aggregate_interval==None:
                    raise sosException.SOSException(2,"Using aggregate functions parameters \"aggregate_interval\" and \"aggregate_function\" are both mandatory")
        ...
        if method == "POST":
            ...
            if len(offs) == 1:
                ...
                if val.nodeType == val.TEXT_NODE:
                    self.offering = get_name_from_urn(str(val.data),"offering")
                else:
            ...
            if len(srss) > 0:
                if len(srss) < 2:
                    ...
                    if val.nodeType == val.TEXT_NODE:
                        self.srsName = get_name_from_urn(str(val.data),"refsystem")
                    else:
            ...
            if len(procs) > 0:
                ...
                for proc in procs:
                    if "xlink:href" in proc.attributes.keys():
                        ...
                        val = proc.firstChild
                        if val.nodeType == val.TEXT_NODE:
                            self.procedure.append(get_name_from_urn(str(val.data),"procedure"))
                    else:
                ...
                if len(fets)<2:
                    ...
                    if len(elements)==1:
                        ...
                        if "xlink:href" in fets[0].attributes.keys():
                            ...
                            if val.nodeType == val.TEXT_NODE:
                                self.featureOfInterest = get_name_from_urn(str(val.data),"feature")
                        else:
