...
def add_params_to_qs(query, params):
    ...
    queryparams = urllib.parse.parse_qsl(query, keep_blank_values=True)
...
def add_params_to_uri(uri, params, fragment=False):
    ...
    sch, net, path, par, query, fra = urllib.parse.urlparse(uri)
    if fragment:
        fra = add_params_to_qs(fra, params)
    else:
        query = add_params_to_qs(query, params)
    return urllib.parse.urlunparse((sch, net, path, par, query, fra))
...
class Request(object):
    ...
    * list of 2-tuples
    ...
    def duplicate_params(self):
        ...
        for k in all_keys:
            seen_keys[k] += 1
        return [k for k, c in list(seen_keys.items()) if c > 1]
